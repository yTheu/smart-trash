...
model = YOLO("../runs/detect/train17/weights/best.pt") # Carrega o modelo
cap =cv2.VideoCapture(0) # Inicia a captura de vídeo

while cap.isOpened():
    ret, frame = cap.read() # Se a captura estiver funcionando, lê o que vê nela
    frame = cv2.resize(frame, (640, 480)) # Redimensionar - menor resolução e maior FPS
    ...
    for result in results:
        for box in result.boxes:
            ...
            label = model.names[int(box.cls)] # Nomes (Person, Cellphone...)
            ...
            if conf > 0.5: # Confiança maior que 50%
                ...
                cv2.putText(frame, f"{label} {conf * 100:.1f}%", (x1, y1-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2)
